...
@Component({
  ...
  template: ` <div
    class="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex items-center justify-center z-[110] p-4"
  >
    <div
      [formGroup]="taskForm"
      class="bg-white w-[98vw] h-[95vh] rounded-xl shadow-2xl flex flex-row overflow-hidden animate-fadeIn font-sans relative"
    >
      <!-- CLOSE BUTTON (Top Right) -->
      <button
        (click)="onClose()"
        class="absolute top-4 right-4 z-50 w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-500 hover:text-gray-800 flex items-center justify-center transition-colors shadow-sm"
      >
      ...
      <div class="flex-1 flex flex-col h-full min-w-0">
        <!-- App Bar / Header -->
        <div
          class="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-white sticky top-0 z-10 shrink-0"
        >
          <div class="flex-1 min-w-0 mr-4">
            <div
              class="flex items-center text-xs font-semibold tracking-wide text-gray-500 mb-1 uppercase truncate"
            >
              ...
              <div *ngIf="!listId" class="relative inline-block ml-1">
                <div
                  (click)="showHierarchyDropdown = !showHierarchyDropdown"
                ...
                <!-- Hierarchy Dropdown -->
                <div
                  *ngIf="showHierarchyDropdown"
                  class="absolute top-full left-0 mt-2 w-72 bg-white border border-gray-200 rounded-lg shadow-xl z-50 p-4 animate-fadeIn flex flex-col gap-3"
                >
                  <!-- 1. Space -->
                  <div class="flex flex-col gap-1">
                    ...
                    <select
                      ...
                      (ngModelChange)="onSpaceSelect($event)"
                    ...
                    >
                      ...
                      <option *ngFor="let s of spaces" [value]="s.id">
                        {{ s.name }}
                      </option>
                    </select>
                  </div>
                  ...
                  <div class="flex flex-col gap-1" *ngIf="selectedSpaceId">
                    ...
                    <select
                      ...
                      (ngModelChange)="onFolderSelect($event)"
                      [ngModelOptions]="{ standalone: true }"
                      class="w-full bg-gray-50 border border-gray-200 rounded px-2 py-1.5 text-xs focus:outline-none focus:border-pink-500 cursor-pointer"
                    >
                      <option [ngValue]="null" disabled>Selecione...</option>
                  ...
                  <div class="flex flex-col gap-1" *ngIf="selectedFolderId">
                    ...
                    <select
                      ...
                      (ngModelChange)="onListSelect($event)"
                      [ngModelOptions]="{ standalone: true }"
                      class="w-full bg-gray-50 border border-gray-200 rounded px-2 py-1.5 text-xs focus:outline-none focus:border-pink-500 cursor-pointer"
                    >
                      <option [ngValue]="null" disabled>Selecione...</option>
                      <option *ngFor="let l of lists" [value]="l.id">
                        {{ l.name }}
                      </option>
                    </select>
                  </div>
                  ...
                  <div
                    *ngIf="selectedListId"
                    class="text-xs text-green-600 font-bold text-center pt-2 border-t border-gray-100"
                  >
                ...
                <div
                  *ngIf="showHierarchyDropdown"
                  (click)="showHierarchyDropdown = false"
                  class="fixed inset-0 z-40 cursor-default"
                ></div>
              </div>

              <span
                *ngIf="taskForm.get('title')?.value"
                class="ml-2 text-gray-400 truncate max-w-[200px]"
                >> {{ taskForm.get('title')?.value }}</span
              >
            </div>
            <input
              type="text"
              ...
              [placeholder]="
                parentTaskId ? 'Título da Subtarefa' : 'Título da Tarefa'
              "
            />
          </div>
        </div>
        ...
        <div
          class="flex-1 overflow-y-auto custom-scrollbar bg-white p-8 space-y-10"
        >
          <!-- Meta Data Section -->
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-6">
              <div class="flex flex-col space-y-2 relative">
                ...
                <div
                  *ngIf="showStatusDropdown"
                  class="absolute top-full left-0 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl z-50 p-1 animate-fadeIn flex flex-col max-h-[300px]"
                >
                  <!-- Search Input -->
                  ...
                  <div class="overflow-y-auto custom-scrollbar flex-1 p-1">
                    ...
                    >
                      ...
                      <i
                        *ngIf="taskForm.get('status')?.value === s.id"
                        class="fas fa-check ml-auto text-pink-500 text-xs"
                      ></i>
                    </div>
                    <div
                      *ngIf="filteredStatusOptions.length === 0"
              ...
              <div class="flex flex-col space-y-2 relative">
                ...
                >
                  <div class="flex items-center">
                    <i
                      class="fas fa-flag mr-2 text-xs"
                      [ngClass]="
                        getPriorityColor(taskForm.get('priority')?.value)
                      "
                    ></i>
                    <span class="text-sm font-medium text-gray-700">{{
                      getPriorityLabel(taskForm.get('priority')?.value)
                    }}</span>
                  </div>
                  <i
                    class="fas fa-chevron-down text-gray-400 text-xs transition-transform"
                    [class.rotate-180]="showPriorityDropdown"
                  ></i>
                </div>
                ...
                <div
                  *ngIf="showPriorityDropdown"
                  class="absolute top-full left-0 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl z-50 p-1 animate-fadeIn"
                >
                  <div
                  ...
                  >
                    <i
                      class="fas fa-flag mr-3 text-xs"
                      [ngClass]="p.colorClass"
                    ></i>
                    <span class="text-sm text-gray-700">{{ p.label }}</span>
                    <i
                      *ngIf="taskForm.get('priority')?.value === p.value"
                      class="fas fa-check ml-auto text-pink-500 text-xs"
                    ></i>
                  </div>
                </div>
              ...
              <div class="flex flex-col space-y-2 relative">
                ...
                >
                  <div
                    *ngIf="selectedTags.length === 0"
                    class="text-gray-400 text-xs font-medium flex items-center"
                  >
                  ...
                  <div
                    *ngFor="let tagId of selectedTags"
                    class="flex items-center px-2 py-1 rounded text-xs font-bold text-white shadow-sm mr-1 mb-1"
                    [style.background-color]="getTagColor(tagId)"
                  >
                    {{ getTagName(tagId) }}
                    <span
                      (click)="$event.stopPropagation(); toggleTag(tagId)"
                      class="ml-1 hover:text-white/70 cursor-pointer"
                      >&times;</span
                    >
                  </div>
                </div>
                ...
                <div
                  *ngIf="showTagDropdown"
                  class="absolute top-full left-0 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl z-50 p-3 w-[260px] animate-fadeIn"
                >
                  <div class="mb-3">
                    <input
                      type="text"
                      ...
                      #tagInput
                      (keyup.enter)="
                        createTag(tagInput.value); tagInput.value = ''
                      "
                      class="w-full bg-gray-50 border border-gray-200 rounded px-2 py-1.5 text-xs focus:outline-none focus:border-pink-500"
                    />
                  </div>
                  ...
                  <div
                    class="max-h-40 overflow-y-auto space-y-1 mb-2 custom-scrollbar"
                  >
                    <div
                      *ngFor="let tag of spaceTags"
                      class="flex items-center justify-between group px-1 py-1 rounded hover:bg-gray-50"
                    >
                      <div
                      ...
                      >
                        ...
                        ></div>
                        <span
                          class="text-sm text-gray-700 truncate max-w-[140px]"
                          >{{ tag.name }}</span
                        >
                        <i
                          *ngIf="selectedTags.includes(tag.id)"
                          class="fas fa-check text-pink-500 ml-auto"
                        ></i>
                      </div>
                  ...
                  <div
                    class="flex gap-1 justify-center border-t pt-2 flex-wrap"
                  >
                    <div
            ...
            <div class="space-y-6">
              <div class="flex flex-col space-y-2">
                ...
                >
                <div
                  class="relative bg-gray-50 rounded-t-lg border-b-2 border-gray-200 hover:border-gray-300 transition-colors flex items-center px-3"
                >
                  <i class="far fa-calendar text-gray-400 mr-3"></i>
                  <input
                    type="date"
                    formControlName="deadline"
                    class="w-full bg-transparent py-3 text-sm text-gray-800 focus:outline-none"
                  />
                </div>
              </div>
              ...
              <div class="flex flex-col space-y-2 relative">
                ...
                >
                  ...
                  <div
                    *ngFor="let userId of selectedAssignees"
                    class="flex items-center bg-pink-100 text-pink-700 px-2 py-1 rounded-full text-xs font-bold mr-1 mb-1"
                  >
                    {{ getMemberName(userId) }}
                    <span
                      (click)="$event.stopPropagation(); toggleAssignee(userId)"
                      class="ml-1 hover:text-pink-900 cursor-pointer"
                      >&times;</span
                    >
                  </div>
                </div>
                ...
                <div
                  *ngIf="showAssigneeDropdown"
                  class="absolute top-full left-0 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl z-50 max-h-48 overflow-y-auto animate-fadeIn"
                >
                  <div
                  ...
                  >
                    <div
                      class="w-4 h-4 border rounded mr-2 flex items-center justify-center transition-colors"
                      [ngClass]="
                    ...
                    >
                      <i
                        *ngIf="isSelected(member.id)"
                        class="fas fa-check text-[10px]"
                      ></i>
                    </div>
                    <div class="flex items-center">
                      <div
                        class="w-6 h-6 rounded-full bg-gray-200 mr-2 overflow-hidden flex items-center justify-center text-[10px]"
                      >
                        <img
                  ...
                  <div
                    *ngIf="members.length === 0"
                    class="p-3 text-xs text-center text-gray-500"
                  >
          ...
          <div class="space-y-3">
            ...
            >
              <i class="fas fa-align-left mr-2 text-pink-600 text-sm"></i>
              Descrição
            </label>
            <div
              class="bg-gray-50 rounded-lg border border-gray-200 focus-within:border-pink-500 focus-within:ring-1 focus-within:ring-pink-200 transition-all"
            >
              <textarea
                formControlName="description"
                class="w-full bg-transparent p-4 text-gray-700 text-base leading-relaxed resize-none focus:outline-none min-h-[140px]"
                placeholder="Adicione detalhes, listas ou contexto para esta tarefa..."
              ></textarea>
            </div>
          </div>
          ...
          <div class="space-y-3">
            ...
            <div
              *ngIf="existingAttachments.length > 0"
              class="grid grid-cols-2 gap-3 mb-3"
            >
              <div
                *ngFor="let att of existingAttachments"
                class="bg-gray-50 border border-gray-200 p-3 rounded-lg flex items-center justify-between group h-fit"
              >
                <a
                  [href]="'' + att.file_url"
                  target="_blank"
                  class="flex items-center overflow-hidden hover:text-pink-600 transition-colors"
                >
                  <div
                    class="w-8 h-8 rounded bg-gray-200 text-gray-500 flex items-center justify-center mr-3 text-xs font-bold uppercase shrink-0"
                  >
            ...
            <div *ngIf="files.length > 0" class="grid grid-cols-2 gap-3 mb-3">
              <div
                *ngFor="let file of files"
                class="bg-blue-50 border border-blue-100 p-3 rounded-lg flex items-center justify-between group h-fit"
              >
                <div class="flex items-center overflow-hidden">
                  <div
                    class="w-8 h-8 rounded bg-blue-100 text-blue-500 flex items-center justify-center mr-3 text-xs font-bold uppercase shrink-0"
                  >
            ...
            <div
              class="border-2 border-dashed border-gray-300 rounded-xl p-8 flex flex-col items-center justify-center text-gray-400 hover:bg-gray-50 hover:border-pink-300 hover:text-pink-500 transition-all cursor-pointer relative"
            >
              <i class="fas fa-cloud-upload-alt text-3xl mb-3"></i>
              <span class="text-sm font-medium"
                >Clique ou arraste arquivos aqui</span
              >
              <input
                type="file"
        ...
        <!-- Footer / Actions -->
        <div
          class="px-8 py-5 border-t border-gray-100 flex justify-end items-center bg-white gap-3 shrink-0"
        >
          <div class="flex gap-3">
            <button
              (click)="onClose()"
              class="px-6 py-2.5 text-gray-600 text-sm font-medium hover:bg-gray-100 rounded-lg transition-colors"
            >
              Cancelar
            </button>
            <button
      ...
      <!-- RIGHT SIDE: ACTIVITY -->
      <div
        class="w-[30%] bg-gray-50 border-l border-gray-200 flex flex-col h-full shrink-0"
      >
        <!-- Activity Header: Added pr-14 to avoid close button overlap -->
        <div
          class="pl-4 pr-14 py-4 border-b border-gray-200 flex items-center justify-between bg-white/50 backdrop-blur-sm"
        >
          <h3 class="text-sm font-bold text-gray-700">Atividade</h3>
          <span class="text-xs text-gray-400"
            >{{ activities.length }} eventos</span
          >
        </div>
        ...
        >
          <div
            *ngIf="activities.length === 0"
            class="text-center text-gray-400 text-xs py-8 italic"
          >
          ...
          <div *ngFor="let log of activities" class="flex gap-3 group">
            <!-- Avatar -->
            <div
              class="w-6 h-6 rounded-full bg-gray-200 shrink-0 overflow-hidden text-[9px] flex items-center justify-center text-gray-500 font-bold"
            >
              <img
                *ngIf="log.user?.avatar_url"
                [src]="'' + log.user.avatar_url"
                class="w-full h-full object-cover"
              />
              <span *ngIf="!log.user?.avatar_url">{{
            ...
            <div class="flex-1">
              ...
              <div class="flex items-baseline justify-between mb-0.5">
                <span class="text-xs font-bold text-gray-700 mr-2">{{
                  log.user?.name
                }}</span>
                <span class="text-[10px] text-gray-400 whitespace-nowrap">{{
                  log.created_at | date: 'd/M HH:mm'
                }}</span>
              </div>

              <!-- Content (System Log vs Comment) -->
              <div
                *ngIf="log.type === 'comment'"
                class="bg-white border border-gray-200 rounded-lg rounded-tl-none p-2.5 text-xs text-gray-700 shadow-sm mt-1"
                [innerHTML]="formatComment(log.content)"
              ></div>
              <div
                *ngIf="log.type === 'system'"
        ...
        <div class="px-4 py-3 bg-white border-t border-gray-200">
          ...
          <div
            class="relative bg-gray-50 rounded-xl border border-gray-200 focus-within:border-pink-500 focus-within:ring-1 focus-within:ring-pink-200 transition-all flex items-center pr-2"
          >
            <textarea
              #commentInput
              (keyup.enter)="
              ...
              class="flex-1 bg-transparent px-3 py-2 text-xs text-gray-700 focus:outline-none resize-none h-[40px] max-h-[80px] custom-scrollbar"
            ></textarea>

            <!-- Dropdown (Moved here for Left Alignment) -->
            <div
              *ngIf="showMentionDropdown"
              class="absolute bottom-full left-0 mb-2 w-48 bg-white border border-gray-200 rounded-lg shadow-xl z-[60] overflow-hidden animate-fadeIn"
            >
              <div class="max-h-48 overflow-y-auto custom-scrollbar">
                <div
                  *ngFor="let member of members"
                  (click)="selectMention(member, commentInput)"
                  class="px-3 py-2 hover:bg-gray-50 cursor-pointer flex items-center text-xs text-gray-700 transition-colors"
                >
                  <!-- Avatar -->
                  <div
                    class="w-5 h-5 rounded-full bg-gray-200 mr-2 overflow-hidden flex items-center justify-center text-[9px] font-bold text-gray-500 shrink-0"
                  >
                    <img
                      *ngIf="member.avatar_url"
                      [src]="'' + member.avatar_url"
            ...
            <!-- Toolbar Actions -->
            <div
              class="flex items-center space-x-3 pl-2 border-l border-gray-200 h-6"
            >
              <!-- Attach -->
              <button
              ...
              <div class="relative">
                <button
                  (click)="insertMention(commentInput)"
                  class="text-gray-500 hover:text-gray-700 transition-colors p-1"
                  title="Marcar membro"
                >
                ...
                <div
                  *ngIf="showMentionDropdown"
                  (click)="showMentionDropdown = false"
              ...
              <!-- Notification Count -->
              <div
                class="flex items-center text-gray-500 cursor-default"
                title="Membros notificados"
              >
...
export class CreateTaskDialogComponent implements OnInit, OnChanges {
  ...
  getCurrentStatusOption() {
    ...
    return (
      this.statusOptions.find((s) => s.id === currentId) ||
  ...
  get selectedSpaceName() {
    return (
      this.spaces.find((s) => s.id === this.selectedSpaceId)?.name ||
      'Selecione o Espaço'
    );
  }
  ...
  get selectedFolderName() {
    return (
      this.folders.find((f) => f.id === this.selectedFolderId)?.name ||
      'Selecione a Pasta'
    );
  }
  ...
  get selectedListName() {
    return (
      this.lists.find((l) => l.id === this.selectedListId)?.name ||
      'Selecione a Lista'
    );
  }
    ...
    this.dataService.getSpaceMembers(targetSpaceId).subscribe({
      next: (members) => {
        this.members = (members || [])
          .map((member: any) =>
            member.user ? { ...member.user, spaceRole: member.role } : member,
          )
  ...
  createTag(name: string) {
    ...
    // Optimistic check?
    const existing = this.spaceTags.find(
    ...
    this.dataService
      .createTag(targetSpaceId, { name, color: this.selectedColor })
      .subscribe({
        next: (newTag) => {
  ...
  deleteTag(tag: any) {
    if (
      confirm(
        `Excluir etiqueta "${tag.name}"? Isso a removerá de todas as tarefas.`,
      )
    ) {
    ...
    if (this.taskForm.valid) {
      ...
      const taskData = {
        ...
        deadline: formValue.deadline
          ? new Date(formValue.deadline).toISOString()
          : null,
      ...
      const request$ = this.task
        ...
        : this.dataService.createTask(targetListId as string, {
            ...taskData,
      ...
      request$.subscribe({
        ...
        error: (err: any) => {
          ...
          alert(
            'Erro ao salvar tarefa: ' + (err.error?.message || err.message),
          );
          this.isSubmitting = false;
        },
      });
  ...
  uploadFiles(taskId: string) {
    ...
    const uploads = this.files.map((file) =>
      this.dataService.uploadAttachment(taskId, file),
    );
