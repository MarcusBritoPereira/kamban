...
@Component({
    ...
    <div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div class="max-w-md w-full space-y-8 bg-white p-8 rounded-lg shadow-sm">
        <div>
          <h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900">Nova Senha</h2>
...
export class ResetPasswordComponent implements OnInit {
    ...
    onSubmit() {
        ...
        this.http.post('http://localhost:3000/v1/auth/reset-password', { token: this.token, password: this.password })
            .subscribe({
                next: () => {
                    this.loading = false;
