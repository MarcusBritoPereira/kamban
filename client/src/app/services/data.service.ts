...
export interface TaskList {
  // Renamed from List to avoid conflict
  id?: string;
  folder_id: string;
  name: string;
  folder?: any;
...
export class DataService {
  ...
  getSpaces(): Observable<Space[]> {
    return this.http
      .get<Space[]>(`${this.apiUrl}/spaces`)
      .pipe(tap((spaces) => this.spaces.set(spaces)));
  }
  ...
  updateSpace(id: string, data: any): Observable<Space> {
    return this.http
      .put<Space>(`${this.apiUrl}/spaces/${id}`, data)
      .pipe(tap(() => this.getSpaces().subscribe()));
  }

  deleteSpace(id: string): Observable<any> {
    return this.http
      .delete(`${this.apiUrl}/spaces/${id}`)
      .pipe(tap(() => this.getSpaces().subscribe()));
  }

  addMember(spaceId: string, email: string): Observable<any> {
    return this.http.post(`${this.apiUrl}/spaces/${spaceId}/members`, {
      email,
    });
  }

  getMySpaceInvitations(): Observable<any[]> {
    return this.http.get<any[]>(`${this.apiUrl}/spaces/invitations/me`);
  }

  acceptSpaceInvitation(token: string): Observable<any> {
    return this.http
      .post(`${this.apiUrl}/spaces/invitations/${token}/accept`, {})
      .pipe(tap(() => this.getSpaces().subscribe()));
  }
  ...
  getLists(folderId: string): Observable<TaskList[]> {
    return this.http.get<TaskList[]>(
      `${this.apiUrl}/folders/${folderId}/lists`,
    );
  }
  ...
  getListTasks(
    ...
    return this.http.get<{ data: Task[]; meta: any }>(
      `${this.apiUrl}/tasks?list_id=${listId}&page=${page}&limit=${limit}`,
    );
  }
  ...
  getMyTasks(
    ...
    return this.http.get<{ data: Task[]; meta: any }>(
      `${this.apiUrl}/tasks/me?page=${page}&limit=${limit}`,
    );
  }
  ...
  getUserTasks(
    ...
    return this.http.get<{ data: Task[]; meta: any }>(
      `${this.apiUrl}/tasks/user/${userId}?page=${page}&limit=${limit}`,
    );
  }
  ...
  createTask(listId: string, data: any): Observable<Task> {
    ...
    return this.http
      .post<Task>(`${this.apiUrl}/tasks`, payload)
      .pipe(tap(() => this.taskUpdatesSubject.next()));
  }

  updateTask(taskId: string, data: any): Observable<Task> {
    return this.http
      .patch<Task>(`${this.apiUrl}/tasks/${taskId}`, data)
      .pipe(tap(() => this.taskUpdatesSubject.next()));
  }
  ...
  addCompanyMember(
    ...
    return this.http.post<any>(
      `${this.apiUrl}/companies/${companyId}/members`,
      { userId, role },
    );
  }
  ...
  removeCompanyMember(companyId: string, userId: string): Observable<any> {
    return this.http.delete<any>(
      `${this.apiUrl}/companies/${companyId}/members/${userId}`,
    );
  }
  ...
  addTaskComment(taskId: string, content: string): Observable<any> {
    return this.http.post(`${this.apiUrl}/tasks/${taskId}/comments`, {
      content,
    });
  }
